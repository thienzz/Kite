...
class Agent:
    ...
    async def run(self, user_input: str, context: Optional[Dict] = None) -> Dict:
        ...
        try:
            ...
            print(f"      [DEBUG] LLM Brain response: {response.strip()[:100]}...")
...
If no tools needed, return []."""
        ...
        try:
            ...
            # Clean JSON
            json_text = raw_extraction
            if "```json" in json_text:
                json_text = json_text.split("```json")[1].split("```")[0].strip()
            elif "```" in json_text:
                json_text = json_text.split("```")[1].split("```")[0].strip()
            
            # Remove any trailing commas or garbage
            ...
            if valid_calls:
                 print(f"      [DEBUG] Final tool calls: {valid_calls}")
                 
            return valid_calls
        except Exception as e:
            # Fallback to simple heuristic if SLM fails or returns garbage
            print(f"      [DEBUG] Tool extraction failed: {e}")
            print(f"      [DEBUG] Raw response was: {raw_extraction if 'raw_extraction' in locals() else 'None'}")
            return []
